    ...
    const ownerMailOptions = {
        ...
        html: `
            <div style="font-family: 'Inter', sans-serif; max-width: 600px; margin: 0 auto; background-color: #050505; color: #ffffff; padding: 40px; border: 1px solid #c9a961;">
                ...
                <div style="grid-template-columns: 1fr 1fr; display: grid; gap: 20px; margin-bottom: 25px;">
                    <div>
                        <p style="margin: 5px 0; color: #c9a961; font-size: 11px; text-transform: uppercase; letter-spacing: 2px;">Budget</p>
                        <p style="margin: 0; font-size: 16px;">${budget || 'Not specified'}</p>
                    </div>
                    <div>
                        <p style="margin: 5px 0; color: #c9a961; font-size: 11px; text-transform: uppercase; letter-spacing: 2px;">Timeline</p>
                        <p style="margin: 0; font-size: 16px;">${timeline || 'Not specified'}</p>
                    </div>
                </div>
...
// 2. Join Circle Endpoint
app.post('/api/join-circle', async (req, res) => {
    ...
    try {
        ...
        console.error('Email Error:', error);
        res.status(500).json({ success: false, message: 'Failed to join the circle' });
    }
});

// 3. General Inquiry Endpoint
